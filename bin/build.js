...
const projectType = redfile.type
const options = {
  cwd: cwd,
  entry: detectEntry(cwd, projectType, redfile.spa),
  devType: projectType,
  spa: redfile.spa
}

if (projectType === 'project' && redfile.spa === false) {
  options.filename = '[name]/index.js'
}

if (projectType === 'component') {
  console.log(`Start ${chalk.cyan('Component')} ${chalk.green('Building')}...`)
  require('../lib/component-build')(options)
} else if (projectType === 'project') {
  console.log(`Start ${chalk.cyan('Project')} ${chalk.green('Building')}...`)
  require('../lib/project-build')(options)
} else {
  console.log('Unknown build type: %s in redfile.json', chalk.red(projectType))
  process.exit(1)
}